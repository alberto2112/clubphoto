...
//-----------------------------------------------------
  function print_row($codalbum, &$photo, $nline, $ratemethod='stars', $selected=false){
     ...
     */
      ...
      echo '<tr>';
        echo '<td><a href="http://'.SITE_DOMAIN.PUBLIC_ROOT.FORMS_DIR.'vote.php?'.URI_QUERY_ALBUM.'='.$codalbum.'&amp;'.URI_QUERY_PHOTO.'='.$photo[0].'"><img src="'.PUBLIC_ROOT.ALBUMS_DIR.$codalbum.'/photos/thumbs/'.$photo[0].'" alt="'.$photo[0].'" /></a></td>';
        echo '<td>'.$photo[3].'</td>';
        echo '<td>'.$votes.'</td>';
        if($ratemethod=='stars'){
          echo '<td>'.$points.'</td>';
...
<!-- / Modal boxes -->
  <div class="header">
      <h1><a class="header-button ico-home notext" href="<?php echo PUBLIC_ROOT.ADMIN_DIR; ?>">Home</a> Classement</h1>
  </div>

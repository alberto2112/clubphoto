...
/** INFOS IMPORTANTES
  ...
  *    est SYSTEM_ROOT.TRASH_DIR.code_album."/index.php"
  ...
  **/
  ...
  //Get $codalbum
  $_CODALBUM       = clear_request_param(getRequest_param(URI_QUERY_ALBUM,''), 'a-zA-Z0-9', 8, false);
...
// Voir s'il y a une demande de clonage de sesion et rediriger utilisateur vers le formulaire correspondant
  ...
  if(is_readable(SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.DIRECTORY_SEPARATOR.PROC_DIR.$LONGIP.'.clnssn')){
  ...
  $ERROR    = new LOG(SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.'/logs/error.log');

  if(@is_readable(SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.'/config.php')===true){
    $AL_CONF = include SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.'/config.php';
/*
    // Recuperer session
    ...
    $_ISMEMBER  = file_exists(SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.'/'.PROC_DIR.$USER_SESSION); //Il a deja eu acces avec le lien privilegie
...
*/
    ...
    $ERROR->insert('ALBUM CONFIG NOT FOUND AT: '.SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.'/config.php', true);
...
<!-- Header -->
    <div class="header">
        <h1><a class="header-button ico-home notext" href="<?php echo PUBLIC_ROOT.ADMIN_DIR.FORMS_DIR; ?>trash.php">Home</a><?php echo get_arr_value($AL_CONF, 'albumname'); ?></h1>
...
<?php
    // Incluire toolbar
    include SYSTEM_ROOT.ADMIN_DIR.FORMS_DIR.'trash_viewer_toolbar.inc';
...
*/
  ...
  // Lire le dossier "thumbs" et composer la gallerie de photos
    foreach(read_dir(SYSTEM_ROOT.TRASH_DIR.$_CODALBUM.'/photos/thumbs/','*.jpg',true) as $file)
    {
      echo '<div class="Image_Wrapper"><a href="'.PUBLIC_ROOT.ADMIN_DIR.FORMS_DIR.'tviewphoto.php?'.URI_QUERY_PHOTO.'='.$file.'&'.URI_QUERY_ALBUM.'='.$_CODALBUM.'"><img src="'.PUBLIC_ROOT.TRASH_DIR.$_CODALBUM.'/photos/medium/'.$file.'" /></a></div>'."\n";
    }
